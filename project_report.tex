...
Raw adjusted prices are downloaded and converted into daily returns. From these, rolling averages, volatility measures and Sharpe-like ratios are computed. Dates are aligned across tickers, missing values are removed, and the final dataset is normalized when necessary.
...
\item \textbf{Hardware} \\
All experiments were executed on a MacBook with Apple Silicon (M1/M2) and 8–16GB RAM. No GPU was required since the project relies on data processing and classical machine-learning tools.
...
\item \textbf{Main settings / hyperparameters} \\
Default \texttt{scikit-learn} configurations were used. The only main parameter explicitly chosen is the rolling window (20 days), which is applied when computing volatility, Sharpe-like metrics and other rolling indicators.
